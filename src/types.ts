...
export interface ApiProfile {
  ...
  /** 异步模式：添加 async=true 查询参数，返回 task_id 后轮询 */
  asyncMode?: boolean
  /** 异步任务 ID 提取路径，默认 'data' 或 'task_id' */
  taskIdPath?: string
  /** 异步轮询间隔（秒），默认 5 */
  pollIntervalSeconds?: number
  providerDrafts?: Partial<Record<ApiProvider, Partial<Pick<ApiProfile, 'baseUrl' | 'model' | 'apiMode' | 'codexCli' | 'apiProxy' | 'responseFormatB64Json' | 'streamImages' | 'streamPartialImages' | 'asyncMode' | 'taskIdPath' | 'pollIntervalSeconds'>>>>
}
